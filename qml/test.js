...
function getData(jsonurl) {
    ...
    xmlhttp.onreadystatechange=function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            myFunctionlc(xmlhttp.responseText);
        }
    }
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
function myFunctionlc(response) {
    var arr = JSON.parse(response);
    var totalobj = arr.length;
    ...
    //var comp = Qt.createQmlObject('import QtQuick 2.4; Component { Text { text: "(" + index + ") " + "zzz" + " " + "vvv" + " " + "kkk" } }', parent);
    ...
    for(i=0;i<totalobj;i++){
        console.log(arr[i][0]);
        console.log(arr[i][1]);
        modelposts.append({"rows": arr[i][0], "cols": arr[i][1]});
    }
